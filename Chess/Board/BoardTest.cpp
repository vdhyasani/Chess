...
BoardTest::BoardTest(const char* fen)
{
    parse_fen(fen);
    spa = spa->getInstance();
...
// parse FEN string
void BoardTest::parse_fen(const char* fen)
{
...
inline int BoardTest::count_bits(uint64_t bitboard)
{
    // bit counter
    int count = 0;
...
inline int BoardTest::get_ls1b_index(uint64_t bitboard)
{
...
inline void BoardTest::add_move(moves *move_list, int move)
{
    // strore move
...
inline int BoardTest::is_square_attacked(int square, int side)
{
    ...
    // attacked by bishops
    if (spa->bishop_attack_bitboard(square, occupancies[both]) & ((side == white) ? bitboards[B] : bitboards[b])) return 1;

    // attacked by rooks
    if (spa->rook_attack_bitboard(square, occupancies[both]) & ((side == white) ? bitboards[R] : bitboards[r])) return 1;

    // attacked by queen
    if ((spa->bishop_attack_bitboard(square, occupancies[both]) | spa->rook_attack_bitboard(square, occupancies[both])) & ((side == white) ? bitboards[Q] : bitboards[q])) return 1;
...
uint64_t BoardTest::mask_knight_attacks(int square)
{
...
uint64_t BoardTest::mask_king_attacks(int square)
{
    // result attacks bitboard
    uint64_t attacks = 0ULL;
...
uint64_t BoardTest::mask_pawn_attacks(int side, int square)
{
    // result attacks bitboard
    uint64_t attacks = 0ULL;
...
void BoardTest::init_leapers_attacks()
{
...
// make move on chess board
inline int BoardTest::make_move2(int move, int move_flag)
{
    // quiet moves
...
// generate all moves
inline void BoardTest::generate_moves(moves *move_list)
{
    // init move count
    move_list->count = 0;
...
// perft driver
U64 BoardTest::perft_driver(int depth, int orig_depth)
{
    ...
    for (int move_count = 0; move_count < move_list->count; move_count++)
    {
        ...
        // make move
        if (!make_move2(move_list->moves[move_count], all_moves))
...
void BoardTest::print_load_bar(int sizeOfMoves, int currentIteration)
{
